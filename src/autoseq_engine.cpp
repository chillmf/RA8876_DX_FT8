...
void autoseq_on_touch(const Decode *msg)
{
    ...
    if (strncmp(msg->field1, Station_Call, sizeof(Station_Call)) == 0)
    {
    ...
    strncpy(ctx->dxcall, msg->field2, sizeof(ctx->dxcall) - 1);
    if (msg->sequence == Seq_Locator) {
       // strncpy(ctx->dxgrid, msg->locator, sizeof(ctx->dxgrid) - 1);
        strncpy(ctx->dxgrid, msg->field3, sizeof(ctx->dxgrid) - 1);
    }
    ctx->snr_tx = msg->snr;
